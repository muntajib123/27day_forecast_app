...
function App() {
  ...
  // Load data from backend -- ORIGINAL: use LSTM predictions
  useEffect(() => {
    (async () => {
      try {
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Box sx={{ backgroundColor: theme.palette.background.default, minHeight: "100vh", py: 6 }}>
        <Container maxWidth="lg" sx={{ bgcolor: theme.palette.background.paper, borderRadius: 4, boxShadow: 5, py: 4, px: { xs: 2, md: 4 } }}>
          <Box display="flex" alignItems="center" justifyContent="center" mb={4}>
            <Box component="img" src="/coralcomp-logo.png" alt="CoralComp Logo" sx={{ height: 60, mr: 2 }} />
            <Typography variant="h3" gutterBottom color="text.primary">🛰️ 27-Day Space Weather Forecast</Typography>
          </Box>
          ...
          <LSTMForecastTable data={filteredData} onDataLoaded={handleDataLoaded} lightMode={true} />

          <Box mt={6}><ForecastCharts data={filteredData} lightMode={true} /></Box>
          <Box mt={6}><ForecastBarCharts data={filteredData} lightMode={true} /></Box>
